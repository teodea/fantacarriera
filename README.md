...
### → **[Walk through the live app](https://fantacarriera.app)** — click *Esplora la Lega*. No signup, no email, nothing to install.
...
Eight people play in it every week. The rules are a written constitution — 41 chapters,
amendable by vote — and the software is its enforcement.
...
Every non-obvious decision is written down as an **ADR** before the code exists — 68 of
them so far, covering things like why money is stored in integer cents, why writes need a
second gate, and why the ledger was split into three.
...
Italian — it is played by an Italian league — but the shape of it reads without the
language.
